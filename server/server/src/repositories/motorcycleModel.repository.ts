...
const mapDbToModelWithBrand = (row: any): ModelWithBrand => ({
  ...mapDbToModel(row),
...
export const createModelRepository = (fastify: FastifyInstance) => {
  ...
  const findAll = async (
    ...
    if (brandId) {
      whereClause += ` WHERE m.brand_id = $${paramCount++}`;
      values.push(brandId);
    }

    if (isCurrent !== undefined) {
      whereClause += whereClause ? " AND" : " WHERE";
      whereClause += ` m.is_current = $${paramCount++}`;
      values.push(isCurrent);
    }

    if (year) {
      whereClause += whereClause ? " AND" : " WHERE";
      whereClause += ` $${paramCount++} BETWEEN m.year_start AND COALESCE(m.year_end, 9999)`;
      values.push(year);
    }

    if (search) {
      whereClause += whereClause ? " AND" : " WHERE";
      whereClause += ` m.model_name ILIKE $${paramCount++}`;
      values.push(`%${search}%`);
    }
   ...
   * Find model by ID
   */
  const findById = async (id: number): Promise<ModelWithBrand | null> => {
    ...
    return result.rows[0] ? mapDbToModelWithBrand(result.rows[0]) : null;
  };
  ...
  /**
   * Get current models (is_current = true)
   */

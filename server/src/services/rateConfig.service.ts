...
const getDefaultConfig = (): RateConfig => ({
  version: "1.0.0",
 ...
 */
export const createRateConfigService = (configPath?: string) => {
  const serviceConfigPath = configPath || path.join(__dirname, "../config/rateConfig.json");
  let cachedConfig: RateConfig | null = null;

  /**
   * Load configuration from file
   */
  const loadConfig = (): RateConfig => {
    try {
      const rawData = fs.readFileSync(serviceConfigPath, "utf-8");
   ...
   */
  const saveConfig = (config: RateConfig): void => {
    try {
      const data = JSON.stringify(config, null, 2);
      fs.writeFileSync(serviceConfigPath, data, "utf-8");
      cachedConfig = config;
    } catch (error) {
   ...
   */
  const getConfigOrLoad = (): RateConfig => {
   ...
   */
  const updateConfig = (updates: UpdateRateConfigDto): RateConfig => {
   ...
   * Update base rate
   */
  const updateBaseRate = (baseRate: Partial<RateConfig["baseRate"]>): RateConfig => {
   ...
   * Get all rate types
   */
  const getRateTypes = (): Record<string, RateTypeConfig> => {
   ...
   */
  const getRateType = (code: string): RateTypeConfig | null => {
    const config = getConfigOrLoad();
   ...
   */
  const createRateType = (data: CreateRateTypeDto): RateTypeConfig => {
    const config = getConfigOrLoad();
    ...
    const newRateType: RateTypeConfig = {
      code: data.code,
      name: data.name,
  ...
  const updateRateType = (
    code: string,
    updates: Partial<RateTypeConfig>,
  ): RateTypeConfig | null => {
    const config = getConfigOrLoad();
   ...
   * Delete rate type
   */
  const deleteRateType = (code: string): boolean => {
    const config = getConfigOrLoad();
   ...
   */
  const getSkillLevels = (): Record<string, SkillLevelConfig> => {
   ...
   */
  const getSkillLevel = (code: string): SkillLevelConfig | null => {
    const config = getConfigOrLoad();
   ...
   */
  const createSkillLevel = (data: CreateSkillLevelDto): SkillLevelConfig => {
    const config = getConfigOrLoad();
    ...
    const newSkillLevel: SkillLevelConfig = {
      code: data.code,
      name: data.name,
      multiplier: data.multiplier,
      description: data.description,
  ...
  const updateSkillLevel = (
    code: string,
    updates: Partial<SkillLevelConfig>,
  ): SkillLevelConfig | null => {
    const config = getConfigOrLoad();
   ...
   * Delete skill level
   */
  const deleteSkillLevel = (code: string): boolean => {
    const config = getConfigOrLoad();
   ...
   */
  const getServiceCategories = (): Record<string, ServiceCategoryConfig> => {
   ...
   */
  const getServiceCategory = (code: string): ServiceCategoryConfig | null => {
    const config = getConfigOrLoad();
   ...
   */
  const createServiceCategory = (data: CreateServiceCategoryDto): ServiceCategoryConfig => {
    const config = getConfigOrLoad();
    ...
    const newCategory: ServiceCategoryConfig = {
      code: data.code,
      name: data.name,
      multiplier: data.multiplier,
      description: data.description,
  ...
  const updateServiceCategory = (
    code: string,
    updates: Partial<ServiceCategoryConfig>,
  ): ServiceCategoryConfig | null => {
    const config = getConfigOrLoad();
   ...
   */
  const deleteServiceCategory = (code: string): boolean => {
    const config = getConfigOrLoad();
   ...
   * Update brand multipliers
   */
  const updateBrandMultipliers = (updates: Partial<RateConfig["brandMultipliers"]>) => {
   ...
   * Update rounding rules
   */
  const updateRoundingRules = (updates: Partial<RateConfig["roundingRules"]>) => {
   ...
   * Update minimum charge rules
   */
  const updateMinimumChargeRules = (updates: Partial<RateConfig["minimumChargeRules"]>) => {
   ...
   */
  const resetToDefault = (defaultConfig: RateConfig): RateConfig => {
...
export const rateConfigService = createRateConfigService();

// Export type
export type RateConfigService = ReturnType<typeof createRateConfigService>;

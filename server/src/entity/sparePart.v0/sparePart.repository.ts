...
function mapDbToPart(row: any): SparePart {
  return {
    id: row.id,
    code: row.code,
    name: row.name,
...
export async function findPartByCode(
  fastify: FastifyInstance,
  code: string,
): Promise<SparePart | null> {
  const { pg } = fastify;
...
export async function createPart(
  ...
  const result = await pg.query(
    `INSERT INTO spare_part (code, name, description, purchase_price, selling_price, markup_multiplier, stock_quantity, supplier, created_by)
...
export async function deletePart(fastify: FastifyInstance, id: number): Promise<boolean> {
  const { pg } = fastify;

...
export const createSparePartRepository = (fastify: FastifyInstance) => {
  const { pg } = fastify;

  /**
   * Find all spare parts with filters
   */
  const findAll = async (params: SparePartQueryParams = {}) => {
    const {
      page = 1,
      limit = 20,
      category,
   ...
   * Find by ID
   */
  const findById = async (id: number): Promise<SparePart | null> => {
   ...
   * Find by code
   */
  const findByCode = async (code: string): Promise<SparePart | null> => {
    const result = await pg.query(
      `SELECT p.*, s.supplier_name
       FROM spare_part_catalog p
       LEFT JOIN suppliers s ON p.primary_supplier_id = s.supplier_id
   ...
   * Find by OEM number
   */
  const findByOem = async (oemNumber: string): Promise<SparePart[]> => {
    const result = await pg.query(
      `SELECT p.*, s.supplier_name
       FROM spare_part_catalog p
       LEFT JOIN suppliers s ON p.primary_supplier_id = s.supplier_id
       WHERE p.oem_part_number = $1 OR $1 = ANY(p.alternative_part_numbers)`,
      [oemNumber],
    );
   ...
   */
  const findLowStock = async (): Promise<SparePart[]> => {
    const result = await pg.query(
      `SELECT p.*, s.supplier_name
       FROM spare_part_catalog p
       LEFT JOIN suppliers s ON p.primary_supplier_id = s.supplier_id
   ...
   */
  const findOutOfStock = async (): Promise<SparePart[]> => {
    const result = await pg.query(
      `SELECT p.*, s.supplier_name
       FROM spare_part_catalog p
       LEFT JOIN suppliers s ON p.primary_supplier_id = s.supplier_id
   ...
   * Find by supplier
   */
  const findBySupplier = async (supplierId: number): Promise<SparePart[]> => {
    const result = await pg.query(
      `SELECT p.*, s.supplier_name
       FROM spare_part_catalog p
       LEFT JOIN suppliers s ON p.primary_supplier_id = s.supplier_id
   ...
   * Find by category
   */
  const findByCategory = async (category: string): Promise<SparePart[]> => {
    const result = await pg.query(
      `SELECT p.*, s.supplier_name
       FROM spare_part_catalog p
       LEFT JOIN suppliers s ON p.primary_supplier_id = s.supplier_id
   ...
   */
  const create = async (data: CreateSparePartDto): Promise<SparePart> => {
    const {
      partCode,
      partName,
      description,
    ...
    const result = await pg.query(
      `INSERT INTO spare_part_catalog (
   ...
   * Update spare part
   */
  const update = async (id: number, data: UpdateSparePartDto): Promise<SparePart | null> => {
    const fields: string[] = [];
    const values = [];
    let paramCount = 1;
   ...
   * Update stock
   */
  const updateStock = async (id: number, movement: StockMovement): Promise<SparePart> => {
   ...
   */
  const isUsedInInvoices = async (id: number): Promise<boolean> => {
  ...
  const remove = async (id: number): Promise<boolean> => {
    const result = await pg.query(
      "DELETE FROM spare_part_catalog WHERE part_id = $1 RETURNING part_id",
      [id],
    );
...
export type SparePartRepository = ReturnType<typeof createSparePartRepository>;

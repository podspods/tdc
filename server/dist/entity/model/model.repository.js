...
exports.findAllModels = findAllModels;
exports.findModelById = findModelById;
exports.findModelsByBrand = findModelsByBrand;
exports.createModel = createModel;
exports.updateModel = updateModel;
exports.deleteModel = deleteModel;
exports.findAllModelInfo = findAllModelInfo;
...
async function findAllModels(fastify, params = {}) {
    const { pg } = fastify;
...
async function findModelById(fastify, id) {
    const { pg } = fastify;
    const result = await pg.query(`SELECT m.*, b.name
     FROM model m
     LEFT JOIN brand b ON m.brand_id = b.id
     WHERE m.id = $1`, [id]);
    return result.rows[0] ? mapDbToModel(result.rows[0]) : null;
}
async function findModelsByBrand(fastify, brandId) {
    const { pg } = fastify;
    const result = await pg.query(`SELECT m.*, b.name
     FROM model m
     LEFT JOIN brand b ON m.brand_id = b.id
     WHERE m.brand_id = $1
     ORDER BY m.year_start DESC`, [brandId]);
    return result.rows.map(mapDbToModel);
}
async function createModel(fastify, data) {
    const { pg } = fastify;
    const { brandId, name, yearStart, yearEnd, isCurrent, engineDisplacement, engineType, powerHp, torqueNm, weightKg, fuelCapacityLiters, description, imageUrl, createdBy, } = data;
    const result = await pg.query(`INSERT INTO model (
...
async function updateModel(fastify, id, data) {
    const { pg } = fastify;
    const fields = [];
    const values = [];
    let idx = 1;
...
async function deleteModel(fastify, id) {
    const { pg } = fastify;
...
// ------------------------------------------------------------------------------------------
async function findAllModelInfo(fastify, params = {}) {
    const { pg } = fastify;
    const { page = 1, limit = 20, brandId, isCurrent, search, minYear, maxYear } = params;

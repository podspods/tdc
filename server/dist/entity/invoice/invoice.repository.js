...
exports.findInvoiceLineByInvoiceId = findInvoiceLineByInvoiceId;
exports.createInvoice = createInvoice;
exports.updateInvoice = updateInvoice;
exports.deleteInvoice = deleteInvoice;
exports.getInvoicesInfoList = getInvoicesInfoList;
exports.createInvoiceLine = createInvoiceLine;
exports.updateInvoiceLine = updateInvoiceLine;
exports.deleteInvoiceLine = deleteInvoiceLine;
function mapDbToInvoiceInfo(row) {
    return {
        id: row.id,
...
function mapDbToInvoiceLine(row) {
    return {
        id: row.id,
...
//--------------------------------------------------------------------------------------------------------------------------
async function findAllInvoices(fastify, params = {}) {
    const { pg } = fastify;
...
//--------------------------------------------------------------------------------------------------------------------------
async function findInvoiceById(fastify, id) {
    const { pg } = fastify;
...
//--------------------------------------------------------------------------------------------------------------------------
async function findInvoiceLineByInvoiceId(fastify, invoiceId) {
    const { pg } = fastify;
...
//--------------------------------------------------------------------------------------------------------------------------
async function createInvoice(fastify, data) {
    console.log("createInvoice data", data);
    const { pg } = fastify;
    const { garageId, vehicleId, invoiceNumber, issueDate, dueDate, statusCode, notes, createdBy } = data;
    console.log("createInvoice 177 typof dueDate", typeof dueDate);
    console.log("createInvoice 177  dueDate", dueDate);
    const result = await pg.query(`INSERT INTO invoice (garage_id, vehicle_id,invoice_number, issue_date, due_date, status_code, notes, created_by)
...
//--------------------------------------------------------------------------------------------------------------------------
async function updateInvoice(fastify, id, data) {
    const { pg } = fastify;
    const fields = [];
    const values = [];
    let idx = 1;
...
//--------------------------------------------------------------------------------------------------------------------------
async function deleteInvoice(fastify, id) {
    const { pg } = fastify;
...
async function getInvoicesInfoList(fastify) {
    const { pg } = fastify;
    const query = `
    SELECT
      i.*,
...
//--------------------------------------------------------------------------------------------------------------------------
// ----- Invoice lines -----
async function createInvoiceLine(fastify, data) {
    const { pg } = fastify;
    const { invoiceId, lineTypeCode, description, quantity, unitPrice, discountRate = 0 } = data;
    const result = await pg.query(`INSERT INTO invoice_line (invoice_id, line_type_code, description, quantity, unit_price, discount_rate)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING *`, [invoiceId, lineTypeCode, description, quantity, unitPrice, discountRate]);
    return mapDbToInvoiceLine(result.rows[0]);
}
//--------------------------------------------------------------------------------------------------------------------------
async function updateInvoiceLine(fastify, lineId, data) {
    const { pg } = fastify;
    const fields = [];
    const values = [];
    let idx = 1;
...
//--------------------------------------------------------------------------------------------------------------------------
async function deleteInvoiceLine(fastify, lineId) {
    const { pg } = fastify;

...
exports.count = count;
async function findAll(fastify, whereClause, values, limit, offset) {
...
async function create(fastify, data) {
    ...
    const updateResult = await fastify.pg.query(`UPDATE part_and_labor SET code = $1 WHERE id = $2 RETURNING *`, [code, newRecord.id]);
...
async function remove(fastify, id) {
    const result = await fastify.pg.query("DELETE FROM part_and_labor WHERE id = $1 RETURNING id", [
        id,
    ]);
    return result.rows.length > 0;
}
async function count(fastify, whereClause, values) {
    const result = await fastify.pg.query(`SELECT COUNT(*) FROM part_and_labor ${whereClause}`, values);
    return parseInt(result.rows[0].count, 10);
}
function mapRowToPartAndLabor(row) {
    return {
        id: row.id,

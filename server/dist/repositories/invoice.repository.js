...
exports.createInvoiceRepository = void 0;
/**
 * Map database snake_case to application camelCase
 */
const mapDbToLaborItem = (row) => ({
...
const mapDbToConsumableItem = (row) => ({
...
const createInvoiceRepository = (fastify) => {
    const { pg } = fastify;
    /**
     * Find all invoices with pagination and filters
     */
    const findAll = async (params = {}) => {
        const { page = 1, limit = 20, registrationId, ownerId, status, fromDate, toDate, overdue, search, } = params;
    ...
    const findById = async (id) => {
        const invoiceResult = await pg.query(`SELECT i.*, 
              r.plate_number, b.brand_name, m.model_name,
              o.first_name as owner_first_name, o.last_name as owner_last_name,
              o.phone_number as owner_phone_number
       FROM invoices i
       ...
       WHERE i.invoice_id = $1`, [id]);
        ...
        // Get labor items
        const laborResult = await pg.query("SELECT * FROM invoice_labor_items WHERE invoice_id = $1 ORDER BY created_at", [id]);
        invoice.laborItems = laborResult.rows.map(mapDbToLaborItem);
        // Get parts items
        const partsResult = await pg.query("SELECT * FROM invoice_parts_items WHERE invoice_id = $1 ORDER BY created_at", [id]);
        invoice.partsItems = partsResult.rows.map(mapDbToPartItem);
        // Get consumable items
        const consumablesResult = await pg.query("SELECT * FROM invoice_consumable_items WHERE invoice_id = $1 ORDER BY created_at", [id]);
        invoice.consumableItems = consumablesResult.rows.map(mapDbToConsumableItem);
        // Get payments
        const paymentsResult = await pg.query("SELECT * FROM invoice_payments WHERE invoice_id = $1 ORDER BY payment_date", [id]);
     ...
     */
    const findByNumber = async (invoiceNumber) => {
    ...
    const create = async (data) => {
        const { registrationId, ownerId, issueDate, dueDate, status, taxRate, notes, internalNotes, createdBy, } = data;
        const result = await pg.query(`INSERT INTO invoices (
      ...
      RETURNING *`, [
        ...
        return findById(invoice.invoiceId);
    };
    /**
     * Update invoice
     */
    ...
    const addLaborItem = async (data) => {
        ...
        const result = await pg.query(`INSERT INTO invoice_labor_items (
        invoice_id, description, hours, rate_per_hour, mechanic_name, notes
      ) VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *`, [invoiceId, description, hours, ratePerHour, mechanicName, notes]);
    ...
    const addPartItem = async (data) => {
        const { invoiceId, partName, partReference, quantity, unitPrice, supplier, warrantyMonths, notes, } = data;
        const result = await pg.query(`INSERT INTO invoice_parts_items (
        invoice_id, part_name, part_reference, quantity, unit_price,
        supplier, warranty_months, notes
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
      RETURNING *`, [invoiceId, partName, partReference, quantity, unitPrice, supplier, warrantyMonths, notes]);
    ...
    const addConsumableItem = async (data) => {
        ...
        const result = await pg.query(`INSERT INTO invoice_consumable_items (
        invoice_id, consumable_name, quantity, unit, unit_price, notes
      ) VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *`, [invoiceId, consumableName, quantity, unit, unitPrice, notes]);
    ...
    const addPayment = async (data) => {
        ...
        try {
            // Insert payment
            const paymentResult = await pg.query(`INSERT INTO invoice_payments (
        ...
        RETURNING *`, [
            ...
            await pg.query(`UPDATE invoices SET 
          ...
          status = CASE 
            ...
            ELSE status
          END
        WHERE invoice_id = $5`, [amount, paymentMethod, reference, paymentDate || new Date(), invoiceId]);
    ...
    const removeLaborItem = async (itemId) => {
        const result = await pg.query("DELETE FROM invoice_labor_items WHERE labor_item_id = $1 RETURNING labor_item_id", [itemId]);
        return result.rowCount ? result.rowCount > 0 : false;
    };
    const removePartItem = async (itemId) => {
        const result = await pg.query("DELETE FROM invoice_parts_items WHERE parts_item_id = $1 RETURNING parts_item_id", [itemId]);
        return result.rowCount ? result.rowCount > 0 : false;
    };
    const removeConsumableItem = async (itemId) => {
        const result = await pg.query("DELETE FROM invoice_consumable_items WHERE consumable_item_id = $1 RETURNING consumable_item_id", [itemId]);
        return result.rowCount ? result.rowCount > 0 : false;
    };
    /**
     * Delete invoice
    ...
    const remove = async (id) => {
        const result = await pg.query("DELETE FROM invoices WHERE invoice_id = $1 RETURNING invoice_id", [id]);
        return result.rowCount ? result.rowCount > 0 : false;
    };
    /**
     * Get statistics
    ...
    const getStats = async () => {
        const totalsResult = await pg.query(`SELECT 
        COUNT(*) as total_invoices,
        COALESCE(SUM(total_amount), 0) as total_amount,
        COALESCE(SUM(amount_paid), 0) as total_paid,
        COALESCE(SUM(amount_due), 0) as total_due
      FROM invoices`);
        const byStatusResult = await pg.query(`SELECT 
        status,
        COUNT(*) as count,
        COALESCE(SUM(total_amount), 0) as total
      FROM invoices
      GROUP BY status`);
        const overdueResult = await pg.query(`SELECT 
        COUNT(*) as count,
...
exports.createInvoiceRepository = createInvoiceRepository;

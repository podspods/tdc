...
exports.createConsumableRepository = void 0;
/**
 * Map database snake_case to application camelCase
 */
const mapDbToConsumable = (row) => ({
...
const createConsumableRepository = (fastify) => {
    const { pg } = fastify;
    /**
    ...
    const findLowStock = async () => {
        const result = await pg.query(`SELECT * FROM consumable_catalog 
    ...
    const create = async (data) => {
        const { consumableCode, consumableName, description, category, unitOfMeasure, packageSize, packageUnit, unitPrice, purchasePrice, supplier, currentStock, minimumStock, maximumStock, reorderPoint, locationInWarehouse, viscosity, specification, safetyDataSheetUrl, hazardous, flammable, createdBy, } = data;
        const result = await pg.query(`INSERT INTO consumable_catalog (
    ...
    const updateStock = async (id, movement) => {
        ...
        try {
            ...
            if (type === "in") {
                // Add stock
                const result = await pg.query(`UPDATE consumable_catalog 
    ...
    const remove = async (id) => {
        const result = await pg.query("DELETE FROM consumable_catalog WHERE consumable_id = $1 RETURNING consumable_id", [id]);
...
exports.createConsumableRepository = createConsumableRepository;

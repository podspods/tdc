...
exports.createModelRepository = void 0;
/**
...
const mapDbToModelWithBrand = (row) => ({
    ...mapDbToModel(row),
    brandName: row.brand_name,
    brandCountry: row.country_of_origin,
});
const createModelRepository = (fastify) => {
    const { pg } = fastify;
    /**
    ...
    const findByBrand = async (brandId) => {
        const result = await pg.query("SELECT * FROM motorcycle_models WHERE brand_id = $1 ORDER BY year_start DESC", [brandId]);
        return result.rows.map(mapDbToModel);
    };
    /**
     * Find model by name
     */
    const findByModelName = async (brandId, modelName) => {
        const result = await pg.query("SELECT * FROM motorcycle_models WHERE brand_id = $1 AND model_name = $2", [brandId, modelName]);
        return result.rows[0] ? mapDbToModel(result.rows[0]) : null;
    };
    /**
     * Create new model
    ...
    const create = async (data) => {
        const { brandId, modelName, yearStart, yearEnd, isCurrent, engineDisplacement, engineType, powerHp, torqueNm, weightKg, fuelCapacityLiters, description, imageUrl, createdBy, } = data;
        const result = await pg.query(`INSERT INTO motorcycle_models (
    ...
    const remove = async (id) => {
        const result = await pg.query("DELETE FROM motorcycle_models WHERE model_id = $1 RETURNING model_id", [id]);
    ...
    const getCurrentModels = async () => {
        const result = await pg.query(`SELECT m.*, b.brand_name 
       FROM motorcycle_models m
       JOIN motorcycle_brands b ON m.brand_id = b.brand_id

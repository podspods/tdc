...
const createRegistrationRepository = (fastify) => {
    const { pg } = fastify;
    /**
    ...
    const findById = async (id) => {
        const result = await pg.query(`SELECT r.*, b.brand_name, m.model_name
       FROM registrations r
       JOIN motorcycle_brands b ON r.brand_id = b.brand_id
       LEFT JOIN motorcycle_models m ON r.model_id = m.model_id
       WHERE r.registration_id = $1`, [id]);
    ...
    const findByPlateNumber = async (plateNumber) => {
       ...
       JOIN motorcycle_brands b ON r.brand_id = b.brand_id
       LEFT JOIN motorcycle_models m ON r.model_id = m.model_id
       WHERE r.plate_number = $1`, [plateNumber]);
        return result.rows[0] ? mapDbToRegistration(result.rows[0]) : null;
    };
    /**
    ...
    const create = async (data) => {
        const { plateNumber, ownerName, ownerPhone, ownerEmail, color, brandId, modelId, createdBy } = data;
        const result = await pg.query(`INSERT INTO registrations (
        plate_number, owner_name, owner_phone, owner_email, color,
        brand_id, model_id, created_by
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
      RETURNING *`, [plateNumber, ownerName, ownerPhone, ownerEmail, color, brandId, modelId, createdBy]);
    ...
    const remove = async (id) => {
        const result = await pg.query("DELETE FROM registrations WHERE registration_id = $1 RETURNING registration_id", [id]);
